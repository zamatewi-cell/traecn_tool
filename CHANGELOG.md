...
### 深度加固与返工收口 (Rework & Hardening)
- **CLI 凭据管道与普通日志严格物理隔离 ([P1-1 返工])**：
  - 引入 `-desktop-ipc` 命令行参数与 `TRAE_DESKTOP_IPC` 环境变量，仅在显式声明时启用跨进程事件帧；普通 CLI 模式 0 凭据输出，彻底阻断文件重定向与日志泄露。
- **刷新凭据全链路事件消费与 UI 防覆盖合并守卫 ([P1-2 返工])**：
  - `preload.js` 补齐 `onAccountUpdated` 接口；`store/index.ts` 订阅并原子更新内存凭据；主进程持久化层实现 `mergeAccountsSafely` 深度合并守卫，防止旧 renderer 快照覆盖主进程最新刷新的 Token。
- **唯一 Account ID 全程精准归属与同名隔离 ([P1-3 返工])**：
  - 全程废弃 `accountName` OR 宽松匹配，全面使用账号稳定唯一 `ID`，彻底消除同名账号凭据覆盖与失败误杀；CLI 未配置 ID 时自动分配稳定 ID。
- **OpenAI Responses 并行工具交错流参数原子隔离 ([P1-4 返工])**：
  - 彻底废弃单一全局 `currentItem`，按 `tool_index` 独立维护元数据容器与参数累加器，完美支持 `工具0 -> 工具1 -> 工具0` 交错输入流与参数原子隔离。
- **消除静态 Token 人为 2 小时强制过期 ([P2-1 收口])**：
  - 移除强制赋予 2 小时有效期的逻辑，还原零值到期时间，由上游 401 真实驱动失效。
- **子进程停止超时防线收敛 ([P2-2 收口])**：
  - `terminateProxyProcess` 超时返回 `{ success: false }` 并保留句柄，防止数据清除守卫误判成功。
- **全链路脱敏校验与恢复拦截 ([P2-3 收口])**：
  - 统一导出格式，导入与恢复对 20+ 种脱敏占位符实施强校验拦截。
- **非 Chat 接口统一透传 tool_choice 并返回标准 400 ([P2-4 收口])**：
  - 统一拦截并返回标准本地 400，严禁包装为 502。
- **限流器 Context 取消 Re-check Loop 消除并发惊群 ([P2-6 收口])**：
  - 引入唤醒重新校准循环，确保 10+ 并发协程下请求间隔严格满足配置要求（非 0ms 放行）。
- **运行时切换当前账号即时确认与一键重启生效 ([P2-7 收口])**：
  - 运行时切换账号增加弹窗确认与一键重启生效机制。
- **全链路版本动态获取彻底消除硬编码与 dev 残留 ([P2-8 收口])**：
  - 统一 CI/CD 标签注入、侧栏与关于页动态获取，彻底消除硬编码 `v1.0.0` 与 `1.0.0-dev.*`。
